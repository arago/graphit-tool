import yaml
from arago.graphit import RestSession
try:
...
class ConnectITView(object):
	...
	def put(self, xid, data):
		return ConnectITTransaction(self, data=self.session.request('PUT', f"/views/{self.id}/objects/{quote_plus(xid)}", data=data).json())

	def get(self, xid):
		return self.session.request('GET', f"/views/{self.id}/objects/{quote_plus(xid)}").json()
